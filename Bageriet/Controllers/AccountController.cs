...
using ExternalLoginConfirmationViewModel = Bageriet.Models.ExternalLoginConfirmationViewModel;
using ForgotPasswordViewModel = Bageriet.Models.ForgotPasswordViewModel;
using LoginViewModel = Bageriet.Models.LoginViewModel;
using RegisterViewModel = Bageriet.Models.RegisterViewModel;
using ResetPasswordViewModel = Bageriet.Models.ResetPasswordViewModel;
using SendCodeViewModel = Bageriet.Models.SendCodeViewModel;
using VerifyCodeViewModel = Bageriet.Models.VerifyCodeViewModel;
...
namespace Bageriet.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        private UserManager<ApplicationUser> UsrManager { get; set; }
        //private ApplicationSignInManager _signInManager;
        //private ApplicationUserManager _userManager;
        ...
        public async Task<ActionResult> Register(RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                if (db.Users.Count() == 1)
                {
                    UsrManager.AddToRole(user.Id, "Admin");
                }
                else
                {
                    UsrManager.AddToRole(user.Id, "Customer");
                }
                if (result.Succeeded)
                {
        ...
        {
            var UserList = db.Users.OrderBy(u => u.UserName).ToList().Select(e => new SelectListItem()
            {
                Text = e.UserName,
                Value = e.UserName
            });
        ...
        public ActionResult GetRoles(string UserName)
        {
            if (!String.IsNullOrEmpty(UserName))
            {
                ...
                this.UsrManager = new UserManager<ApplicationUser>(new UserStore<ApplicationUser>(this.db));
                ViewBag.RolesForThisUser = UsrManager.GetRoles(user.Id);
